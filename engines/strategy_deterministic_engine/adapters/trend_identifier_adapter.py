...
class TrendIdentifierKiteAdapter:
    def __init__(
        ...
        self._fo_universe_symbols: Optional[List[str]] = None
    ...
    def get_fo_universe_symbols(self) -> List[str]:
        if self._fo_universe_symbols is not None:
            return list(self._fo_universe_symbols)
        ...
        if fut.empty:
            raise TrendIdentifierAdapterError("No futures symbols found in NFO instruments")

        fut["name"] = fut["name"].fillna("").astype(str).str.upper()

        stock_fut = fut[fut["name"].isin(nse_equity_symbols)].copy()
        index_fut = fut[~fut["name"].isin(nse_equity_symbols)].copy()

        combined = pd.concat([stock_fut, index_fut], ignore_index=True)
        symbols = sorted(combined["name"].dropna().astype(str).str.upper().unique().tolist())

        if not symbols:
            raise TrendIdentifierAdapterError("No F&O universe symbols found")

        self._fo_universe_symbols = symbols
        return list(symbols)
    ...
    def _count_trading_sessions(self, selection_day: date, expiry_day: date) -> int:
    ...
    def get_contract_info_for_symbol(self, symbol: str, asof_time: object) -> FOSymbolContractInfo:
        symbol = symbol.upper()
    ...
    def build_strategy_input_for_symbol(
        ...
        symbol = symbol.upper()
        fo_symbols = set(self.get_fo_universe_symbols())
    ...
    def build_strategy_inputs_for_fo_universe(
        ...
        symbols = self.get_fo_universe_symbols()
