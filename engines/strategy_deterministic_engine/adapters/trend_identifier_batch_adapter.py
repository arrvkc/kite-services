...
class TrendIdentifierBatchAdapter:
    def __init__(
        ...
        if missing_trend_columns:
            raise BatchAdapterError(f"Missing trend history columns: {sorted(missing_trend_columns)}")
        if missing_contract_columns:
            raise BatchAdapterError(f"Missing contract snapshot columns: {sorted(missing_contract_columns)}")
