#!/usr/bin/env python3
"""
Equity stop history what-if simulator.

Primary mode:
- User gives symbol and entry date
...
2. Explicit entry price:
   PYTHONPATH=.:services python engines/stop_engine/equity_stop_history_whatif.py ACC --exchange NSE --entry-date 2025-12-26 --entry-price 2453.86
...
4. End date override:
   PYTHONPATH=.:services python engines/stop_engine/equity_stop_history_whatif.py ACC --exchange NSE --entry-date 2025-12-26 --end-date 2026-04-10

5. CSV-style output:
   PYTHONPATH=.:services python engines/stop_engine/equity_stop_history_whatif.py ACC --exchange NSE --entry-date 2025-12-26 --format csv
...
def find_equity_instrument(kite: KiteConnect, exchange: str, symbol: str) -> Dict:
    ...
    if not matches:
        raise ValueError(f"No {exchange} instrument found for symbol={symbol}")

    # Prefer EQ series when present.
    eq_matches = [m for m in matches if str(m.get("segment") or "").upper().endswith("EQ") or str(m.get("instrument_type") or "").upper() in {"EQ", ""}]
    if len(eq_matches) == 1:
        return eq_matches[0]
    if eq_matches:
        return eq_matches[0]
...
def main() -> int:
    ...
    instrument = find_equity_instrument(kite, args.exchange, args.symbol)
    ...
    if args.show_summary:
        print("EQUITY STOP HISTORY WHAT-IF")
